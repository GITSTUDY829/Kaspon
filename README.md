...
<img width="956" height="416" alt="image" src="https://github.com/user-attachments/assets/41be1ef6-fbe7-4319-9e77-873c3e1177ea" />
<img width="957" height="415" alt="image" src="https://github.com/user-attachments/assets/a831bed0-fb2e-458b-9fbc-b66d7c34ecee" />
<img width="958" height="416" alt="image" src="https://github.com/user-attachments/assets/7cd41bc9-4ea8-45b9-ac22-a7b03f8abad8" />
...
1. **<span dir="ltr">Supabase</span>:** הרץ את `setup.sql` ב‑<span dir="ltr">SQL Editor</span> — הוא יוצר את הכול (טבלאות `transactions` + `profiles`, אינדקסים, <span dir="ltr">RLS</span>, וטריגר אישור המשתמשים) במכה אחת.
...
index.css               — העיצוב (RTL)
─── Backend (Supabase) ───
setup.sql                         — סכמת ה-DB: טבלאות transactions + profiles, אינדקסים, RLS, וטריגר אישור
edge-function-log-transaction.ts  — Edge Function: פענוח SMS + סיווג + המרת מטבע
```

---

## 🛠️ תהליך הבנייה (<span dir="ltr">Vibe Coding</span>)

כספון נבנה מקצה לקצה בעבודה צמודה עם כלי <span dir="ltr">AI</span> כשותף פיתוח — מתכנון הארכיטקטורה ועד דיבוג בזמן אמת:

- **אבטחה** — מודל ההרשאות תוכנן בשיח עם ה‑<span dir="ltr">AI</span>: מדיניות <span dir="ltr">RLS</span> שמבטיחה שכל משתמש ניגש רק לנתונים שלו, שמירת מפתח ה‑<span dir="ltr">service_role</span> אך ורק בתוך ה‑<span dir="ltr">Edge Function</span> (לעולם לא בצד הלקוח), ומנגנון אישור משתמשים עם טריגר ב‑<span dir="ltr">DB</span>.
- **<span dir="ltr">Edge Function</span>** — הפונקציה שמפענחת הודעות <span dir="ltr">SMS</span> משלוש חברות אשראי ישראליות נכתבה ושופרה באיטרציות: אימות קלט בצד השרת, חילוץ סכום / בית עסק / תאריך, סיווג אוטומטי, והמרת מטבע זר לשקלים — תוך הקפדה שה‑<span dir="ltr">Shortcut</span> לעולם לא נוגע ישירות בבסיס הנתונים.
- **פתרון תקלות** — ה‑<span dir="ltr">AI</span> סייע לאבחן באגים אמיתיים צעד‑אחר‑צעד: שגיאת <span dir="ltr">"network connection lost"</span> באייפון שמקורה התגלה כתווים מיוחדים במפתח הסודי, ה‑<span dir="ltr">toggle</span> של <span dir="ltr">Verify JWT</span> שחוזר ונדלק אחרי כל פריסה של ה‑<span dir="ltr">Edge Function</span>, ובניית <span dir="ltr">Vite</span> שדרשה מבנה ריפו שטוח כדי לעבוד נכון ב‑<span dir="ltr">Vercel</span>.

הגישה אפשרה להתמקד בהבנת הבעיה ובחוויית המשתמש, בזמן שה‑<span dir="ltr">AI</span> תרגם החלטות לקוד עובד וקיצר משמעותית את לולאת הדיבוג.
